const input: &'static str = r#"nkzjrdqrmpztpqninetwofour1znnkd
...
fn main() -> () {
    ...
    let mut currNumString = String::new();
    for char in input.chars() {
        if char == '\n' {
            println!("{}", currNumString);
            let mut curr = currNumString.parse::<u64>().unwrap_or(0);
            if (curr < 10) {
                curr += curr * 10;
            }
            output += curr;
            currNumString = String::new();
        }
        if char.is_digit(10) {
            if currNumString.chars().count() < 2 {
                currNumString.push(char);
            } else {
                currNumString.pop();
                currNumString.push(char);
            }
